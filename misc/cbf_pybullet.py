...
class KukaPointRobotEnv(PointRobotEnv):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.client.setAdditionalSearchPath(pybullet_data.getDataPath())
        # Place Kuka at the bottom of the bounding box and scale it up to cover the space
        self.kuka = self.client.loadURDF("kuka_iiwa/model.urdf", basePosition=[0, 0, -1], globalScaling=1.5)
        self.num_joints = self.client.getNumJoints(self.kuka)
        self.end_effector_index = 6

    def step(self):
        # Find position of the green point robot
        green_pos = self.client.getBasePositionAndOrientation(self.robot)[0]
        
        # Calculate inverse kinematics for Kuka end effector
        jointPoses = self.client.calculateInverseKinematics(
            self.kuka,
...
def main():
    ...
    env = KukaPointRobotEnv(config.pos_min, config.pos_max)
