...
def run_tests(model="llama3.1:70b"):
    ...
    # Common PnP trajectory parameters
    pnp_target_start = [0.45, -0.5, 0.55]
    pnp_waypoints = [
        [0.45, -0.5, 0.55],  # t=0.5s: Start above pick location
    ...
    pnp_times = [0.5, 2.0, 4.0, 7.0, 9.0]

    # 1. Dynamic Motion
    dynamic_motion = {
        "experiment_title": f"Dynamic_Motion_{model_name}",
        "ee_start": [0.24, 0.0, 0.429],
        "target_start": pnp_target_start,
        "waypoints": pnp_waypoints,
        "times": pnp_times,
    ...
    for job in all_jobs:
        ...
        # Generate new PnP prompt
        prompt = bar.create_prompt_pnp(
            ee_pos=job["ee_start"],
            targ_pos=job["target_start"],
            waypoint=job["waypoints"],
            timestep=job["times"],
        ...
        try:
            ...
            ee_min, ee_max, wb_min, wb_max = bar.generate_barrier(
                user_prompt=prompt, model_name=model, sin_traj=False
            )
            ...
            output_path = os.path.join(job["output_dir"], "llm_res", f"pnp_{model_name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run PnP LLM barrier tests.")
    parser.add_argument(
        "--model",
        type=str,
