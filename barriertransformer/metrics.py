...
class SimulationData:
    ...
    collision_spheres: np.ndarray = (
        None  # Shape: (num_obstacles, 3) or (T, num_obstacles, 3)
    )
...
@jax.jit
def compute_svr_wb(
    joint_spheres: jnp.ndarray,
    joint_sphere_radii: jnp.ndarray,
    box_min: jnp.ndarray,
    box_max: jnp.ndarray,
):
...
@jax.jit
def compute_cia(
    collision_spheres: jnp.ndarray,
    collision_sphere_radii: jnp.ndarray,
    box_min: jnp.ndarray,
    box_max: jnp.ndarray,
):
...
@jax.jit
def compute_bact(
    u_actual: jnp.ndarray, u_nominal: jnp.ndarray, dt: float, epsilon: float = 1e-4
):
...
def generate_report(data: SimulationData, output_dir: str = "results"):
    ...
    if (
        data.p_target is not None
        and data.pos_min is not None
        and data.pos_max is not None
    ):
        svr_ee = float(
            compute_svr_ee(
                jnp.array(data.p_target),
                jnp.array(data.pos_min),
                jnp.array(data.pos_max),
            )
        )
    else:
        svr_ee = 0.0
    ...
    if (
        data.collision_spheres is not None
        and data.collision_sphere_radii is not None
        and data.pos_min is not None
        and data.pos_max is not None
    ):
        cia_ee_cumul, cia_ee_peak = compute_cia(
            jnp.array(data.collision_spheres),
            jnp.array(data.collision_sphere_radii),
            jnp.array(data.pos_min),
            jnp.array(data.pos_max),
        )
    ...
    if has_wb:
        if data.joint_spheres is not None and data.joint_sphere_radii is not None:
            svr_wb = float(
                compute_svr_wb(
                    jnp.array(data.joint_spheres),
                    jnp.array(data.joint_sphere_radii),
                    jnp.array(data.wb_min),
                    jnp.array(data.wb_max),
                )
            )
        else:
            svr_wb = None
        ...
        if (
            ...
            cia_wb_cumul, cia_wb_peak = compute_cia(
                jnp.array(data.collision_spheres),
                jnp.array(data.collision_sphere_radii),
                jnp.array(data.wb_min),
                jnp.array(data.wb_max),
    ...
    # Volume (BAR)
    ee_vol = (
        float(compute_bar(jnp.array(data.pos_min), jnp.array(data.pos_max)))
        if data.pos_min is not None and data.pos_max is not None
        else 0.0
    )
    wb_vol = (
        float(compute_bar(jnp.array(data.wb_min), jnp.array(data.wb_max)))
        if has_wb
        else None
    )

    # Control Metrics
    bact_rate, bact_dur = compute_bact(
        jnp.array(data.u_actual), jnp.array(data.u_nominal), data.dt
    )
    ...
    if data.p_actual is not None and data.p_target is not None:
        ...
        mte_mean_all, mte_std_all, mte_mean_last_10s, mte_std_last_10s = compute_mte(
            jnp.array(data.p_actual), jnp.array(data.p_target), steps_10s=steps_10s
        )
        mte_mean_all, mte_std_all = float(mte_mean_all), float(mte_std_all)
        mte_mean_last_10s, mte_std_last_10s = (
            float(mte_mean_last_10s),
            float(mte_std_last_10s),
        )
    else:
        mte_mean_all, mte_std_all, mte_mean_last_10s, mte_std_last_10s = (
            0.0,
            0.0,
            0.0,
            0.0,
...
def save_barriers_to_csv(data: SimulationData, output_dir: str = "results"):
    ...
    ee_min_str = (
        str(data.pos_min.tolist())
        if hasattr(data.pos_min, "tolist")
        else str(data.pos_min)
    )
    ee_max_str = (
        str(data.pos_max.tolist())
        if hasattr(data.pos_max, "tolist")
        else str(data.pos_max)
    )
    wb_min_str = (
        str(data.wb_min.tolist())
        if hasattr(data.wb_min, "tolist")
        else str(data.wb_min)
    )
    wb_max_str = (
        str(data.wb_max.tolist())
        if hasattr(data.wb_max, "tolist")
        else str(data.wb_max)
    )
